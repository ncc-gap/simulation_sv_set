...
def cutesvSVtoBedpe(input_vcf, output, f_grc, filter_scaffold_option, bcf_filter_option):
    ...
    run_vcf2bedpe(out_pref + ".tmp1.vcf", out_pref + ".tmp1.bedpe", "STRAND")
...
def simplify_cutesv(in_control_bedpe, hout, h_chrom_number):
...
def cutesvSVtoBedpe_main(args):
    
    in_tumor_sv = args.in_cutesv_tumor_sv
    in_control_sv = args.in_cutesv_control_sv
    margin = args.margin
    f_grc = args.f_grc
    ...
    cutesvSVtoBedpe(in_tumor_sv, output_prefix+'.cutesv_tumor_PASS.bedpe', f_grc, filter_scaffold_option, bcf_filter_option)

    repair_dup_strand(output_prefix+'.cutesv_tumor_PASS.bedpe', output_prefix+'.cutesv_tumor_repaired.bedpe')

    cutesvSVtoBedpe(in_control_sv, output_prefix+'.cutesv_control_PASS.bedpe', f_grc, filter_scaffold_option, bcf_filter_option)

    repair_dup_strand(output_prefix+'.cutesv_control_PASS.bedpe', output_prefix+'.cutesv_control_repaired.bedpe')

    bcftools_command = ["bcftools", "view", "-h", in_tumor_sv, "-o", output_prefix +'.cutesv.vcf.header']
    subprocess.check_call(bcftools_command)
    h_chrom_number = utils.make_chrom_number_dict(output_prefix +'.cutesv.vcf.header')

    with open(output_prefix+'.cutesv_control_simplify.bedpe', 'w') as hout:
        simplify_cutesv(output_prefix+'.cutesv_control_repaired.bedpe', hout, h_chrom_number)

    with open( output_prefix +'.cutesv_control_sorted.bedpe', 'w') as hout:
        subprocess.check_call(['sort', '-k1,1', '-k2,2n', '-k4,4', '-k5,5n', '-k9,9', '-k10,10',  output_prefix +'.cutesv_control_simplify.bedpe'],  stdout = hout)

    with open(output_prefix +'.cutesv_control_sorted.bedpe.gz', "w") as hout:
        subprocess.check_call(["bgzip", "-f", "-c", output_prefix +'.cutesv_control_sorted.bedpe'], stdout = hout)
    subprocess.check_call(["tabix", "-p", "bed", output_prefix +'.cutesv_control_sorted.bedpe.gz'])
          
    filt_clustered_rearrangement2(output_prefix+'.cutesv_tumor_repaired.bedpe', output_prefix+'.cutesv_filtered.txt', 
    output_prefix+'.cutesv_control_sorted.bedpe.gz', margin, min_tumor_support_read, max_control_support_read, min_sv_length, h_chrom_number)

    with open(output_prefix + ".cutesv_sorted.txt", 'w') as hout:
        subprocess.check_call(["sort", "-k1,1", "-k2,2n", "-k4,4", "-k5,5n", "-k3,3", "-k6,6", output_prefix + ".cutesv_filtered.txt"],  stdout = hout)
    ...
    with open(output, 'w') as hout:
        ...
        with open(output_prefix + ".cutesv_sorted.txt", 'r') as hin:
            for line in hin:
                print(line.rstrip('\n'), file=hout)
            
    if not debug:
        os.remove(output_prefix +'.cutesv_tumor_PASS.bedpe')
        os.remove(output_prefix +'.cutesv_control_PASS.bedpe')
        os.remove(output_prefix +'.cutesv_tumor_repaired.bedpe')
        os.remove(output_prefix +'.cutesv_control_repaired.bedpe')
        os.remove(output_prefix +'.cutesv.vcf.header')
        os.remove(output_prefix +'.cutesv_control_simplify.bedpe')
        os.remove(output_prefix +'.cutesv_control_sorted.bedpe')
        os.remove(output_prefix +'.cutesv_filtered.txt')
        os.remove(output_prefix +'.cutesv_sorted.txt')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--in_cutesv_tumor_sv", help = "the vcf format file", type = str, required=True)
    parser.add_argument("--in_cutesv_control_sv", help = "the vcf format file", type = str, required=True)
