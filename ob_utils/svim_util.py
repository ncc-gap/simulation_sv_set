...
def svimSVtoBedpe_main(args):
    ...
    with open(output_prefix +'.svim_sorted.txt', 'w') as hout:
        subprocess.check_call(["sort", "-k1,1", "-k2,2n", "-k4,4", "-k5,5n", "-k3,3", "-k6,6", output_prefix + ".svim_filtered.txt"],  stdout = hout)
    ...
    with open(output, 'w') as hout:
        ...
        with open(output_prefix + ".svim_sorted.txt", 'r') as hin:
            for line in hin:
                print(line.rstrip('\n'), file=hout)
    ...
    if not debug:
        ...
        os.remove(output_prefix +'.svim_sorted.txt')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--in_svim_tumor_sv", help = "the vcf format file", type = str, required=True)
    parser.add_argument("--in_svim_control_sv", help = "the vcf format file", type = str, required=True)
